...
class BasisTest : public ::testing::Test
{
  protected:
    BasisTest()
      : epsilonBasis{ new NDFA{} }
      , symbolBasis{ new NDFA{ 's' } }
    {
...
{
    EXPECT_TRUE(
      epsilonBasis->getStart().hasEpsilonTransitionTo(epsilonBasis->getEnd()));
    EXPECT_TRUE(
      symbolBasis->getStart().hasSymbolTransitionTo(symbolBasis->getEnd()));
}
...
TEST_F(BasisTest, concatOperation)
{
    ...
    const std::vector<State> result =
      epsilonBasis->getStart()[Transition::epsilon];
    ...
    EXPECT_EQ(result[0][Transition::epsilon][0]['s'][0],
              epsilonBasis->getEnd());
}
...
class MatcherTest
  : public ::testing::WithParamInterface<InputValue>
  , public ::testing::Test
{
  protected:
    MatcherTest()
      : regexToNDFA{ new NDFA{ GetParam().regex } }
    {}
...
INSTANTIATE_TEST_CASE_P(Default,
                        MatcherTest,
                        testing::Values(InputValue("abc", "abc"),
