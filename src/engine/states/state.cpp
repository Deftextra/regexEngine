...
State_ptr Automata::State::create(std::string name)
{
  return std::make_shared<State>(State{name});
}

State_ptr Automata::State::addTransition()
{
...
State_ptr Automata::State::addTransition(char symbol)
{
  State_ptr end = create();
...
void Automata::State::addTransition(Transition t)
{
  transitions.push_back(t);
}

void Automata::State::addTransition(State_ptr toState)
{
  transitions.push_back(Transition::create(toState));
}

void Automata::State::addTransition(char symbol,State_ptr toState)
{
  transitions.push_back(Transition::create(symbol, toState));
}

std::vector<State> Automata::State::operator[](char symbol) const
{
...
std::vector<State> Automata::State::operator[](Transition::Type t) const
{
  std::vector<State> states;
...
std::vector<State_ptr> Automata::State::getEndsFrom(char symbol)
{
...
std::vector<State_ptr> Automata::State::getEndsFrom(Transition::Type t)
{
  std::vector<State_ptr> states;
...
bool Automata::State::hasEpsilonTransitionTo(State_ptr state)
{
...
bool Automata::State::hasSymbolTransitionTo(State_ptr state)
{
...
bool Automata::State::hasSymbolTransitionTo(const State& state)
{
...
bool Automata::operator==(const State& lhs, const State& rhs)
{
  return lhs.transitions == rhs.transitions; 
}

bool Automata::State::operator==(const State& rhs)
{
  return transitions == rhs.transitions;
}

std::ostream& Automata::operator<<(std::ostream& os, const State& s)
{
